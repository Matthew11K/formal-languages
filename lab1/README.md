...
\[
\texttt{caacb} \xRightarrow{(1)} \texttt{accb}
\]


\[
\texttt{caacb} \xRightarrow{(2)} \texttt{caadb} \xRightarrow{(1)} \texttt{acdb}
\]
...
\[
\boxed{(5)\ \ \texttt{accb} \to \texttt{acdb}}
\]
...
\[
\texttt{daaacb} \xRightarrow{(3)} \texttt{adcb}
\]

\[
\texttt{daaacb} \xRightarrow{(2)} \texttt{daaadb} \xRightarrow{(3)} \texttt{addb}
\]
...
\[
\boxed{(6)\ \ \texttt{adcb} \to \texttt{addb}}
\]
...
\[
\boxed{(7)\ \ \texttt{bdaa} \to \texttt{bac}}
\]
...
\[
\boxed{(8)\ \ \texttt{acccb} \to \texttt{accdb}}
\]
...
\[
\boxed{\,X\,\texttt{cdcb}\ \to\ X\,\texttt{cddb}\,}
\]
...
например, \[
\texttt{a dcb} \xRightarrow{(6)} \texttt{addb}.
\]
...
- для любого \(k \ge 0\): \(\texttt{d}^k\,\texttt{cdcb} \to \texttt{d}^k\,\texttt{cddb}\);

- для любых \(m,n \ge 0\): \(\texttt{c}^m \texttt{d}^n\,\texttt{cdcb} \to \texttt{c}^m \texttt{d}^n\,\texttt{cddb}\);

- вообще, для всех \(X \in \{c, d\}^\ast:\)  
  \[
  X\,\texttt{cdcb} \;\to\; X\,\texttt{cddb}.
  \]
...
4. Остаточные профили по модулю m. 
   ...
   ( \(k_0\) - до первого `b`, \(k_p\) - после последнего `b` ).
    ...
    Для \(M\ge2\) определим остаточный профиль
