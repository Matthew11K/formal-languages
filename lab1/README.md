...
$$
\\underline{bd}\\ldots \\xRightarrow{(4)} \\underline{bc}\\ldots
$$
...
- **Если $t$ нечётно**, то применяем $(1)$ к последнему `c` ровно $\\lfloor t/2\\rfloor$ раз и убываем
...
$$
b\\,c\\,a^{2n}b \\xRightarrow{(1)\\times n} b\\,a^{n}c\\,b \\xRightarrow{(2)} b\\,a^{n}d\\,b \\xRightarrow{(3)\\times n} b\\,d\\,a^{3n}b \\xRightarrow{(4)} b\\,c\\,a^{3n}b
$$
...
Рассмотрим семейство слов $c^n$, $n\\ge 1$.
...
Переориентируем правила так, чтобы они убывали по shortlex:
```text
caa -> ac
acb -> adb
daaa -> ad
...
$$
\\text{caacb} \\xRightarrow{(1)} \\text{accb}
$$


$$
\\text{caacb} \\xRightarrow{(2)} \\text{caadb} \\xRightarrow{(1)} \\text{acdb}
$$

Нормальные формы различны: `accb` $\\neq$ `acdb`.
Сравнение по shortlex: длины равны (=4), на 3-й позиции `c>d`, значит `accb > acdb`.

Добавляем:

$$
\\boxed{(5)\\;\\;\\text{accb}\\to\\text{acdb}}
$$
...
$$
\\text{daaacb} \\xRightarrow{(3)} \\text{adcb}
$$

$$
\\text{daaacb} \\xRightarrow{(2)} \\text{daaadb} \\xRightarrow{(3)} \\text{addb}
$$

Нормальные формы различны: `adcb` $\\neq$ `addb`.  
Shortlex: длины равны (=4), на 3-й позиции `c>d`, значит `adcb > addb`.  

Добавляем:

$$
\\boxed{(6)\\;\\;\\text{adcb}\\to\\text{addb}}
$$
...
$$
\\texttt{bcaa} \xRightarrow{(4)} \texttt{bdaa}
$$

$$
\\texttt{bcaa} \xRightarrow{(1)} \texttt{bac}
$$

Нормальные формы различны: `bdaa` $\\neq$ `bac`.  
Shortlex: длины 4>3, значит `bdaa > bac`.  

Добавляем:

$$
\\boxed{(7)\\;\\;\\text{bdaa}\\to\\text{bac}}
$$
...
$$
\texttt{caaccb} \xRightarrow{(1)} \texttt{acccb}
$$

$$
\texttt{caaccb} \xRightarrow{(5)} \texttt{caacdb} \xRightarrow{(1)} \texttt{accdb}
$$

Разные НФ: `acccb` $\\neq$ `accdb`.  
Shortlex: длины равны, на 4-й позиции `c>d` значит `acccb > accdb`.

Добавляем:

$$
\\boxed{(8)\\;\\;\\text{acccb}\\to\\text{accdb}}
$$
...
например, $\\text{a dcb}\\Rightarrow\\text{addb}$. 
...
- для любого $k\\ge 0$: $\\text{d}^k\\,\\text{cdcb}\\to\\text{d}^k\\,\\text{cddb}$;

- для любых $m,n\\ge 0$: $\\text{c}^m\\text{d}^n\\,\\text{cdcb}\\to\\text{c}^m\\text{d}^n\\,\\text{cddb}$;

- вообще, для всех $X\\in\\{c,d\\}^\*$: $X\\,\\text{cdcb}\\to X\\,\\text{cddb}$.
...
4. Остаточные профили по модулю m. 
    ...
    Пусть $k(w)=(k_0,k_1,\\dots,k_p)$, где $k_i$ - длина блока из $\\{c,d\\}$ между соседними `b`
   ( $k_0$ - до первого `b`, $k_p$ - после последнего `b` ).

    Для $M\\ge 2$ определим остаточный профиль

$$
r_M(w)=\\big(k_0\\bmod M,\\;k_1\\bmod M,\\;\\dots,\\;k_p\\bmod M\\big).
$$
